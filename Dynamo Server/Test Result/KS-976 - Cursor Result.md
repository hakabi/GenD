...
| **Canonical inventory** | `Dynamo Server/Test Guide/dynamo-mcp-testing-guide.md` section 1.3 (13 tools, April 2026) |

---

## 1. Objective (from ticket)

Confirm that **every tool listed in guide section 1.3** is **registered and visible** to the MCP client so that functional and security tests cover the deployed surface.

**Expected tool names (13):**

| # | Tool | section 1.4 high-risk | Descriptor in `mcps/.../tools/` | Agent invoke smoke |
| ---: | --- | --- | :---: | :---: |
...
**Comparison to section 1.3:** **13/13** tool names present in the Cursor MCP cache; **no extra** tool files; **no missing** tools.
...
| Item | Observation |
| --- | --- |
| **Workspace MCP config** | `.cursor/mcp.json` currently has `"mcpServers": {}` — **Conceptia Dynamo is configured in Cursor UI (user MCP settings)**, not in the repo file for this snapshot. SSE URL per prior setup: `https://mcp.conceptia.com/dynamo/sse` via `mcp-remote` (guide section 2.4). |
...
1. **Filesystem enumeration** — Listed `mcps/user-conceptia-dynamo/tools/*.json` and compared names to section 1.3.
...
4. **section 1.4 tracking** — Confirmed high-risk tools **`list_table`**, **`describe_table`**, **`read_data`** are present in inventory; one high-risk tool (`list_table`) smoke-called successfully.
...
| --- | --- | --- |
| All 13 section 1.3 names visible in Cursor MCP cache | **Pass** | 13 matching `*.json` files under `mcps/user-conceptia-dynamo/tools/`. |
| Enumeration matches canonical table (no missing/extra) | **Pass** | Set equality with section 1.3 list. |
| Agent can invoke MCP tools | **Pass** | `get_funds` and `list_table` returned successful responses. |
| section 1.4 high-risk tools listed and trackable | **Pass** | `list_table`, `describe_table`, `read_data` present; `list_table` smoke **Pass**. |
...
### 4.2 BDD scenarios (ticket / guide section 3.3, Cursor only)
...
| **2 — Error path** (0 tools → escalate per section 9) | **N/A** | Tools enumerated; not triggered. |
| **3 — Edge case** (new tool / drift vs section 1.3) | **Pass (no drift)** | No unexpected tools; if the vendor adds a 14th tool later, re-run discovery per guide. |

---

## 5. Follow-up (outside this run)

1. **Second client:** Run the same enumeration on **Claude Code** (`/mcp`) and attach output to KS-976 for the multi-client matrix (guide section 2.4).
2. **Optional:** Capture Cursor **Settings → MCP** screenshot or “N tools enabled” string for audit trail.
3. **Downstream:** section 1.4 tools (`list_table`, `describe_table`, `read_data`) remain in scope for **KS-981** and security stories — this ticket only confirms they are **registered and visible**.
...
| QA guide reference | `Dynamo Server/Test Guide/dynamo-mcp-testing-guide.md` section 1.3, section 1.4, section 3.3 |
