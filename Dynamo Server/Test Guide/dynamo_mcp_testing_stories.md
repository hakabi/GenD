...
> **Prepared:** 2026-04-21 · **Last updated:** 2026-05-07 (section 1.3 and section 1.4 updated — 6 tools confirmed removed as intentional production hardening; aligned to testing guide v1.3)  
...
| Dimension | Summary |
|-----------|---------|
| **What is under test** | The Conceptia Dynamo MCP server (HTTP/SSE), Microsoft OAuth (Azure AD), and **7 registered tools** (6 removed as intentional production hardening, confirmed 2026-05-07) — validated **only through the MCP surface** (no separate product UI for truth). |
...
| **Tool inventory (7 active / 6 removed)** | **Active (7):** `analyze_notes`, `get_activity`, `get_documents`, `get_fund_description`, `get_funds`, `get_notes`, `llm_text_analysis`. **Removed — intentional production hardening (2026-05-07) (6):** ~~`describe_table`~~, ~~`get_rating_details`~~, ~~`get_rating_summary`~~, ~~`list_table`~~, ~~`read_data`~~, ~~`search_aloha_funds`~~. |
| **High security risk (guide section 1.4)** | **`list_table`**, **`describe_table`**, **`read_data`** — **REMOVED from server (intentional production hardening, confirmed 2026-05-07)**. All section 1.4 scenarios recorded as S (Skipped). FINDING-04 closed as remediated. RECALL suite (10 cases) confirmed no bypass path exists. See [KS-981] comment ID 20211. |
...
**Overview:**
Validates section 3.3: enumeration matches the canonical table (**7 tools active as of 2026-05-07**; 6 removed as intentional production hardening).

**Detailed Requirements:**
- Compare visible tool list to section 1.3 table; flag any missing/extra tools to vendor.
- Verify **7 active tools**: `analyze_notes`, `get_activity`, `get_documents`, `get_fund_description`, `get_funds`, `get_notes`, `llm_text_analysis`.
- **Removed tools (6 — intentional production hardening, confirmed 2026-05-07):** `describe_table`, `get_rating_details`, `get_rating_summary`, `list_table`, `read_data`, `search_aloha_funds`. These will not appear in the tool registry. If any re-appears, trigger E2 discovery re-run immediately.
- Optional prompt: *"List every tool available from the conceptia-dynamo MCP server."*
- Note **section 1.4 high-risk tools** (`list_table`, `describe_table`, `read_data`) have been **removed from server** — see [KS-981] comment ID 20211 for FINDING-04 closure details.
...
*Generated: 2026-04-21 · Updated: 2026-05-07 · Source: `dynamo-mcp-testing-guide.md` **v1.3** (black-box MCP, section 1.4 high-risk tools, Antigravity coverage) · Template: BA Skill*  
*2026-05-07 update: section 1.3 tool inventory revised from 13 to 7 active tools. 6 tools confirmed removed as intentional production hardening (list_table, describe_table, read_data, search_aloha_funds, get_rating_details, get_rating_summary). section 1.4 HIGH-risk tools all removed. FINDING-04 closed. KS-981 comment ID 20211, KS-982 comment ID 20212, KS-978 comment ID 20213.*  
