...
- **Action:** Execute the final step of the pom-to-csharp-page skill to output the complete C# file.
...
- **Inputs:** 
  ...
  - **[INPUT 3] Project Constitution (`constitution.md`):** Serves as the standard guideline for code formatting, testing principles, and project conventions to ensure all generated scripts are uniform and compliant.
  - **[INPUT 4] Reference Templates:** Must adhere to the exact code structure, assertion styles, logging and wait handling shown in the standard project template files (`SeleniumWorkbenchApp\Pages\RiskModelPage.cs` and `SeleniumWorkbenchApp\Tests\Features Testing\Regression Testing\RiskModelTest.cs`).
