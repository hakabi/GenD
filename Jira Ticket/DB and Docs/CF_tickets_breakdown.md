...
> **Total Tickets:** 15 active stories *(CF-13 cancelled; CF-8 superseded by CF-16)*
...
> **Generated:** 2026-04-13 · **Refreshed:** 2026-07-02 (KS-1045/1049/1050, KS-966 superseded, KS-970 CSV)  
> **Location:** `Jira Ticket/DB and Docs/` · **Handoff:** [`cash_forecast_session_handoff.md`](./cash_forecast_session_handoff.md)
...
| CF-12 | [`KS-970`](https://gendvn.atlassian.net/browse/KS-970) | Cash Forecast - Implement Future Transactions Table with Filters and **CSV** Export on Details Tab |
...
| CF-15 | [`KS-1049`](https://gendvn.atlassian.net/browse/KS-1049) | Cash Forecast - Implement Dashboard Sub-Tab Export to Excel (.xlsx) |
| CF-16 | [`KS-1050`](https://gendvn.atlassian.net/browse/KS-1050) | Cash Forecast - Remove Dashboard As-Of Date Picker and Fix to Current Run Day |
...
Sprint 3 (Dashboard advanced — depends on CF-4 ([`KS-962`](https://gendvn.atlassian.net/browse/KS-962)), CF-5 ([`KS-963`](https://gendvn.atlassian.net/browse/KS-963))):
  ...
  CF-8 ([`KS-966`](https://gendvn.atlassian.net/browse/KS-966))  Historical As-Of-Date Lookup  ⚠️ SUPERSEDED by CF-16
...
Sprint 5 (Details Tab):
  CF-12 ([`KS-970`](https://gendvn.atlassian.net/browse/KS-970)) Transactions Table with Filters & CSV Export

Sprint 6 (Jul 2026 — PO change requests):
  CF-15 ([`KS-1049`](https://gendvn.atlassian.net/browse/KS-1049)) Dashboard Export to Excel
  CF-16 ([`KS-1050`](https://gendvn.atlassian.net/browse/KS-1050)) Remove standalone As of Date section
```
...
> ⚠️ **SUPERSEDED (Jul 2026):** Kathleen Bui confirmed removal of the standalone As of Date section on Dashboard ([`KS-1050`](https://gendvn.atlassian.net/browse/KS-1050), comment **20592**). Partial as-of replay was not technically feasible (chart not stored; Tuan table on KS-1045). **Cancel KS-966** after KS-1050 ships. **Retain** read-only as-of subtitle on KS-960 Fixed Income card header only.
...
**Ticket Title:** `Cash Forecast - Implement Future Transactions Table with Filters and CSV Export on Details Tab`
...
- A **"Download to CSV"** button exports the currently filtered table to a `.csv` file. Filename format: `cash_forecast_transactions_[YYYY-MM-DD].csv`.
- The table must support at least 500 rows without performance degradation (implement pagination or virtual scrolling).

**UI/UX & Front-End Considerations:**
- **Layout:** Full-width table on the Details sub-tab. Filter controls in a row above the table. "Download to CSV" button top-right.
...
- **Accessibility:** Table must use correct `<table>` semantics. Download button must have `aria-label="Download transactions as CSV file"`. Filter dropdowns must be keyboard-navigable.

**Acceptance Criteria (BDD Format):**

*Scenario 1 — Happy Path: User filters by asset class and exports*
- **Given** the Details tab has loaded with 200 transactions
- **When** the user selects "Venture Capital" from the Asset Class filter and clicks "Download to CSV"
- **Then** only Venture Capital transactions are shown in the table, and the downloaded file is named `cash_forecast_transactions_[today's date].csv` containing only the filtered rows with all columns including `beta`, `beta_contribution`, `beta_impact`
...
- **Then** the hypothetical trade appears in the transactions table with a visual badge (e.g., "Hypothetical") distinguishing it from real transactions, and it is included in the CSV export
...
## CF-15 ([`KS-1049`](https://gendvn.atlassian.net/browse/KS-1049)) — Dashboard Sub-Tab: Export to Excel (.xlsx)

**Ticket Title:** `Cash Forecast - Implement Dashboard Sub-Tab Export to Excel (.xlsx)`

**Epic:** Cash Forecasting Model (KS-950)

**User Story:**
> As a **Portfolio Manager**, I want to download the current Cash Forecast Dashboard as an Excel workbook so that I can archive today's forecast when full historical replay is not available.

**Overview:**
Implements Jerry Luo's request (KS-1045, 2026-06-26). Export is **Dashboard sub-tab only** — not Historical or Details. Kathleen confirmed (20590): use Excel to archive the **current** Dashboard version when full as-of replay is deferred. After **Calculate Impact**, export must reflect recalculated state.

**Detailed Requirements:**
- Add **"Export to Excel"** (or equivalent) on the Dashboard sub-tab.
- Export format: **`.xlsx`**.
- Include all visible Dashboard sections: **KS-960** (Fixed Income card — including card header as-of subtitle), **KS-962** (Projected Balance Chart data), **KS-963** (Hypothetical Flows if visible), **KS-965** (Projected Cash Flow drill-down data as shown).
- **Not in scope:** Historical tab export; Details tab (uses **CSV** on KS-970).
- Filename: product-standard pattern with run date (e.g. `cash_forecast_dashboard_[YYYY-MM-DD].xlsx`).

**Acceptance Criteria (summary):**
- Export produces valid `.xlsx` with Dashboard sections populated from current view.
- Post–Calculate Impact export matches recalculated charts/tables.

**Traceability:** KS-1045 Item 9 → KS-1049 · Relates KS-1045

---
---

## CF-16 ([`KS-1050`](https://gendvn.atlassian.net/browse/KS-1050)) — Dashboard: Remove Standalone As of Date Section

**Ticket Title:** `Cash Forecast - Remove Dashboard As-Of Date Picker and Fix to Current Run Day`

**Epic:** Cash Forecasting Model (KS-950)

**User Story:**
> As a **Portfolio Manager**, I want the Dashboard to always show today's cash forecast run so that I am not misled by partial historical as-of behaviour.

**Overview:**
Kathleen confirmed (20592): remove the **entire standalone "As of Date" section** — date picker, Return to Today, "Viewing as of" banner, and related label block. Dashboard defaults to **current run day**. **Retain** the read-only as-of subtitle on the **KS-960 Fixed Income card header** (not part of the removed section). Historical and Details tab date pickers are **unchanged**. **Cancel KS-966** after this ships.

**Detailed Requirements:**
- Remove standalone As of Date UI from Dashboard.
- Dashboard charts/summary always reflect **today's** morning run (or post–Calculate Impact state).
- **Do not remove** KS-960 card header as-of subtitle (read-only, today).
- Historical tab (KS-967–969) and Details tab (KS-970) date controls unaffected.

**Acceptance Criteria (summary):**
- No standalone as-of picker, banner, or Return to Today on Dashboard.
- KS-960 card header subtitle still displays correctly.

**Traceability:** KS-1045 Item 10 → KS-1050 · Supersedes **CF-8 / KS-966**
...
| Historical lookback window (CF-8 ([`KS-966`](https://gendvn.atlassian.net/browse/KS-966))) | **SUPERSEDED (Jul 2026)** — standalone Dashboard as-of removed (**CF-16 / KS-1050**); cancel KS-966 | N/A |
| Dashboard Excel archive (CF-15 ([`KS-1049`](https://gendvn.atlassian.net/browse/KS-1049))) | **`.xlsx`** on Dashboard only; no Historical export | Full replay may be future |
| Details export (CF-12 ([`KS-970`](https://gendvn.atlassian.net/browse/KS-970))) | **CSV** (not Excel) | — |
...
*Generated: 2026-04-13 · Refreshed: 2026-07-02 · Location: `Jira Ticket/DB and Docs/` · **15 active stories** + CF-13 cancelled + CF-8 superseded · See [`cash_forecast_session_handoff.md`](./cash_forecast_session_handoff.md)*
