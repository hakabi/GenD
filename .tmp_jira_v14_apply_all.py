...
payload_dir = Path(__file__).with_name(".tmp_jira_v14_payloads")
...
for key in mod.KEYS:
    ...
    if not desc_file.exists():
        summary.append({"key": key, "status": "failed", "note": "description file missing"})
        continue
    existing = desc_file.read_text(encoding="utf-8")
